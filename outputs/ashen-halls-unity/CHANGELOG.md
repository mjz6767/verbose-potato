...
- Preserved the v2.12 Red Gate campaign and save schema v25 while promoting six accepted runtime-art pins from the v2.12.1, v2.13, v2.14, and v2.15 presentation slices. The integrated manifest names 54 exact runtime pins and the candidate package is expected to contain 89 approved PNGs.
- This entry describes the integrated source candidate, not completed release evidence. RuleSmoke, full RuntimeBoot, focused UI/art/audio checks, clean Windows build and package, clean-extracted boot, final-binary visual/audio review, physical-controller review, and a complete human Chapter I-V playthrough remain required before promotion.
