...
- Added deterministic keyboard and controller navigation: W/S or vertical input moves the current card, left/right input pages long detail text, Home/End and Page Up/Page Down traverse long books, 1-4 select filters, Tab or controller shoulders cycle them, Enter/Space/A activates, and Esc/B returns to the board. Selection scrolls fully into view and focus returns to the command deck.
