...
The bounded kobold Chapter II route is normal playable content. Other prototype classes, encounters, formulas, services, factions, dungeons, and generic route scaffolds remain gated behind the full prototype content set. Existing saves retain their serialized map dimensions, biome layout, party stats, talents, and unspent growth points; save schema remains v25. Grand Hearth Art v2.7.0 is the current development candidate after RuleSmoke, focused SpriteArtRuntimeSmoke, full RuntimeBoot, the final Windows build, and six supported-resolution built-player captures pass. Package inventory, clean-extracted packaged boot, physical-controller review, and complete human playthrough remain pending. Town Hall Gathering v2.6.0 remains the verified packaged rollback baseline.
