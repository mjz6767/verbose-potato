...
For v2.15, automated release checks confirm that required keys and track names exist, all 161 imported SFX resolve over procedural fallbacks, all 54 music masters resolve over their procedural score contexts, every declared preview matches its recorded digest/format/duration/peak contract, samples are finite and bounded, representative combat layering stays inside its peak budget, loops meet endpoint and boundary-window budgets, and semantically different files do not collapse to identical waveforms. Human release QA must still listen through the complete title loop, title interaction preview, World Map preview, combat preview, mute/unmute continuity, and representative routed transitions at 25%, 65%, and 100% music/SFX volume for masking, repetition, spatial readability, transition balance, small-speaker clarity, and clean headroom.
