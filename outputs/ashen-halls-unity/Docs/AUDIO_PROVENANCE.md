...
## v2.16.0 route identity and preview-contract hardening

This pass keeps the 54-cue score inventory and every runtime route key stable while removing four reused composition blueprints. `Steel against the Chosen` no longer shares generic combat's BPM, tonal center, mode, progression, and palette combination; the Gloam Deep crypt, Red Gate seal, and Ash Fen ancient grove site cues likewise no longer reuse their matching region/landmark blueprints. Their deterministic Unity fallbacks carry corresponding tempo, pitch, and phrase differences, so missing imported masters preserve the same route distinctions.

The exact 20-second World Map QA mix now exercises the runtime sequence rather than holding one static bed: local road begins at 0.0 seconds, `Ashen Atlas` enters at 4.5 seconds through the 1.10-second World Map equal-power fade, and `Footsteps Behind` enters at 12.0 seconds through the immediate 0.85-second exploration fade. Representative tab, wayfinding, wind, and encounter cues remain on the default SFX source gain. The title and combat QA mixes remain exact 20-second default-gain composites.

`python Tools/Audio/BuildOriginalAudio.py` writes the v2.16 score medley, title master, and three runtime previews under `QA/ash-and-brimstone-v2.16.0-*-preview.wav`. Validation now rejects reused BPM/root/mode/progression/palette blueprints, enforces the same 15-30.1 second non-title loop window used by Unity's rule/runtime smoke checks, verifies the preview's music/SFX gains and World Map/elite route bindings against the C# runtime sources, verifies its World Map and exploration fade durations, and records measured peak, RMS, and headroom beside each preview digest. Runtime composites must retain at least 3 dBFS peak headroom and remain inside a broad -42 to -18 dBFS RMS window; those limits are intentionally wide enough to detect silence or sustained overload without matching one waveform.

All regenerated material remains original deterministic local synthesis from the existing oscillator, resonator, colored-noise, envelope, and delay-network process. No recording, sample pack, model-produced audio, generative-audio service, network service, or external input contributed to this pass. `Tools/Audio/TestOriginalAudioContracts.py` independently checks blueprint uniqueness, equal-power math, route order, source alignment, and the non-fragile level window.
...
For v2.16, automated release checks confirm that required keys and track names exist, all 161 imported SFX resolve over procedural fallbacks, all 54 music masters resolve over their procedural score contexts, composition blueprints are unique, preview source gains/routes/fade durations match the runtime constants, every declared preview matches its recorded digest/format/duration/peak/RMS contract, samples are finite and bounded, representative combat layering stays inside its peak budget, and loops meet endpoint and boundary-window budgets. Human release QA must still listen through the complete title loop, title interaction preview, routed World Map preview, combat preview, mute/unmute continuity, and representative in-game transitions at 25%, 65%, and 100% music/SFX volume for masking, repetition, spatial readability, transition balance, small-speaker clarity, and clean headroom.
