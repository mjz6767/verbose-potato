...
`v2.0.0` is the current family-playtest candidate for the Midgaard sewer and bounded Old Road slice. It turns warlock Demon Form into a complete combat stance with its own claw attack and four Demon Arts, adds Rift Bolt and Rift Step to Pact progression, expands both power-art atlases, and gives the new abilities distinct tier-three visual and audio identities. The v1.99 combat-command and book focus model remains intact: Preview, Selection, focus, and Targeting stay separate, and the bottom command changes truthfully between Spellbook and Demon Arts as the form begins and ends. RuleSmoke, focused combat-UI runtime smoke, full RuntimeBoot, the Windows build/package, and clean-extracted packaged boot pass. Physical-controller review, subjective effect/audio review, and a complete real playthrough from a clean extraction remain release gates. Save schema remains v25. It is not a content-complete game.
...
- The v2.0.0 source, player, and editor assemblies compile directly from Unity's generated response files. Unity RuleSmoke, focused combat-UI runtime smoke, full RuntimeBoot, the Windows build/package, and clean-extracted packaged exploration boot also pass in Unity 6000.3.18f1.
