...
`v2.1.0` is the current family-playtest candidate for the Midgaard sewer and bounded Old Road slice. It makes combat readable at a glance with illustrated active/target dossiers, six portrait initiative chips, explicit target provenance, a full-height tactical rail, larger battlefield HP/status markers, quieter target washes, and a rebalanced command deck. Pointer Preview, committed Selection, keyboard/controller focus, and armed Targeting remain separate in both combat and the power books. The v2.0 Demon Form stance, Rift Bolt, Rift Step, exact power atlases, and distinct Demon Art feedback remain intact. RuleSmoke, focused combat-UI runtime smoke, full RuntimeBoot, the Windows build/package, and clean-extracted packaged boot pass. Physical-controller review, subjective effect/audio review, and a complete real playthrough from a clean extraction remain release gates. Save schema remains v25. It is not a content-complete game.
...
- The v2.1.0 source, player, and editor assemblies compile directly from Unity's generated response files. Unity RuleSmoke, focused combat-UI runtime smoke, full RuntimeBoot, the Windows build/package, and clean-extracted packaged exploration boot also pass in Unity 6000.3.18f1.
- The v2.1 rule smoke, focused combat-UI smoke, runtime boot smoke, and Windows build/package are the latest complete Unity 6000.3.18f1 gate set, with no `Attempting to select ... while already selecting` warning. Runtime coverage exercises portraits, turn chips, target provenance, two compact Timeline rows, pointer-only command preview, explicit controller focus, modal focus cleanup, passive power-book Preview, selected and targeting rails, and Reduced Motion's static impact contract.
- The Windows packaging tool retains the prior good zip until the candidate zip clean-extracts, boots the exact packaged v2.1.0 exploration path in batch mode, exits successfully, and writes a release-integrity manifest with the committed release-source fingerprint plus package, executable, and every `Docs/PACKAGED_ART.txt` PNG SHA-256 hash. Final packaging rejects dirty release inputs, untracked package-selected art, source/staging hash drift, and packaged-art-manifest drift.
