...
`v1.99.0` is the current source candidate for the Midgaard sewer and bounded Old Road slice. It rebuilds the bottom combat-command strip around large readable emblems, responsive focusable controls, and separate semantic, focused, unavailable, selected, and genuinely armed states. Cast and Skills show the exact pending formula or ability only while it is armed, and both power books give selected art a larger responsive detail hero. The source, player, and editor assemblies compile directly from Unity's generated response files. Six advisory development-player captures at 1280x720 and 1920x1080 exit cleanly and visually confirm the representative command and book states. Unity RuleSmoke, RuntimeBoot, Windows build/package, and clean-extracted packaged boot are not passed for v1.99 because the local Unity Editor session currently reports no valid license before those suites begin. v1.98 remains the last fully gated package. Physical-controller review and a complete real playthrough from a clean extraction remain release gates. Combat mechanics and save schema v25 are unchanged. It is not a content-complete game.
...
- The v1.99 source, player, and editor assemblies compile directly from Unity's generated response files. This proves the current code and editor checks compile, but it is not a substitute for executing Unity RuleSmoke, RuntimeBoot, the focused combat-UI smoke, or a player build.
- The v1.98 rule smoke, focused combat-UI smoke, runtime boot smoke, and Windows build/package remain the latest complete Unity 6000.3.18f1 gate set, with no `Attempting to select ... while already selecting` warning. Runtime coverage exercises passive preview and row commit in both books, mouse/controller handoff in the command deck, committed-row restoration in Inventory, all 49 production formula art cells, all 18 live skill art cells, and the typed power-state precedence/icon table.
- The Windows packaging tool retains the prior good zip until the candidate zip clean-extracts, boots the exact packaged v1.98 exploration path in batch mode, exits successfully, and writes a release-integrity manifest with the committed release-source fingerprint plus package, executable, and all 74 `Docs/PACKAGED_ART.txt` PNG SHA-256 hashes. Final packaging rejects dirty release inputs, untracked package-selected art, source/staging hash drift, and packaged-art-manifest drift.
...
- Restore or reactivate the local Unity Editor license, then run v1.99 RuleSmoke, focused combat-UI smoke, RuntimeBoot, the Windows build/package, clean-extracted packaged boot, and the deterministic capture packet. The current license failure occurs before the test methods execute, so none of those v1.99 gates may be counted as passed yet.
- Repeat the six-state v1.99 combat/book visual matrix in the real packaged build after the Unity license is restored. The disposable development-player review is clean but remains advisory rather than packaged-release evidence.
...
- One complete real playthrough from a clean extracted folder remains the release gate before calling this broadly shareable; the v1.98 automated clean-extract boot passes independently, while the equivalent v1.99 gate is still license-blocked.
