...
- For v2.6, Town Hall-aware RuleSmoke, focused sprite-art runtime smoke, full RuntimeBoot, the current Windows build, and direct Local and Region visual review at 1280 by 720 and 1920 by 1080 pass. Every capture under `QA/v2.6.0-town-hall` reports `complete=True`, `failure=None`. Before promotion, still require final packaging, clean-extracted packaged boot, physical-controller review, and a complete New Game -> Muster -> Town Hall -> Midgaard walkthrough. Preserve the verified v2.5 package as rollback evidence until every remaining gate passes; save schema remains v25.
...
- For v2.6, confirm the clean-extract boot log reports the exact `v2.6.0 / save 25` marker and reaches the Town Hall first-spawn exploration state. Confirm the adjacent integrity manifest records `sourceDirty=false`, the final release commit, `cleanExtractLaunch=true`, player exit code 0, and the promoted ZIP, executable, and packaged-art hashes. The v2.4 citizen atlas must remain the single packaged source for both procedural exterior citizens and authored Town Hall patron presentation; no new patron atlas is expected.
