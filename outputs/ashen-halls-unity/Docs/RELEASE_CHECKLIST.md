...
- For v2.5, keep the candidate explicitly in development until growth-aware RuleSmoke, focused runtime smoke, full RuntimeBoot, Windows build/package, clean-extracted packaged boot, and the complete growth visual/manual matrix pass from one settled source state. Confirm the release still reports save schema v25 and preserve the verified v2.4 logs, manifest, and package as rollback evidence until promotion.
...
- Capture the staged Party Growth preview with `-ashen-growth-smoke` at 960x600, 1280x720, and 1920x1080. Confirm five tabs fit the strip; the selected member, available points, staged Strength, and class-relevant talent agree; before/after values are readable; Apply and Reset remain visible; and no row, badge, footer, or detail copy clips.
